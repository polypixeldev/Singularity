module.exports = {
    ...
    async slashExecute(client, Discord, interaction, serverDoc){
        ...
        .setColor(0x000000)
        .setTitle('Singularity General Commands')
        .setFooter(`General help requested by ${interaction.user.tag} • ${currentDate.getUTCMonth()}/${currentDate.getUTCDate()}/${currentDate.getUTCFullYear()} @ ${currentDate.getUTCHours()}:${currentDate.getUTCMinutes()} UTC`, interaction.user.displayAvatarURL());

        let modEmbed = new Discord.MessageEmbed().setColor(0x000000)
        .setColor(0x000000)
        .setTitle('Singularity Moderation Commands')
        .setFooter(`Moderation help requested by ${interaction.user.tag} • ${currentDate.getUTCMonth()}/${currentDate.getUTCDate()}/${currentDate.getUTCFullYear()} @ ${currentDate.getUTCHours()}:${currentDate.getUTCMinutes()} UTC`, interaction.user.displayAvatarURL());

        let msEmbed = new Discord.MessageEmbed().setColor(0x000000)
        .setColor(0x000000)
        .setTitle('My Singularity Commands')
        .setFooter(`My Singularity help requested by ${interaction.user.tag} • ${currentDate.getUTCMonth()}/${currentDate.getUTCDate()}/${currentDate.getUTCFullYear()} @ ${currentDate.getUTCHours()}:${currentDate.getUTCMinutes()} UTC`, interaction.user.displayAvatarURL());
       ...
       if(interaction.options.get('type')?.value === 'general'){
        ...
        `)
        .addFields({name: 'General Help', value: `\`${serverDoc.prefix}help general\``, inline: true}, {name: 'Moderation Help', value: `\`${serverDoc.prefix}help mod\``, inline: true}, {name: 'My Singularity Help', value: `\`${serverDoc.prefix}help ms\``, inline: true})
        .setFooter(`Help requested by ${interaction.user.tag} • ${currentDate.getUTCMonth()}/${currentDate.getUTCDate()}/${currentDate.getUTCFullYear()} @ ${currentDate.getUTCHours()}:${currentDate.getUTCMinutes()} UTC`, interaction.user.displayAvatarURL());
