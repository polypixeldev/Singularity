module.exports = {
	name: 'set',
	description: 'Set the protons/electrons/dark matter of other My Singularity users',
	type: 'ms',
	options: [{
		name: 'user',
		...
		name: 'value',
		description: 'The new value - prefix it with + or - to add or subtract from the current value',
		type: 'STRING',
		required: true
	}],
	args: [],
	...
	async execute(client, Discord, msg, args, serverDoc){
		...
		client.utils.updateUser(client, serverDoc.guildID, userMS.userID, userMS)
		.then(() => {
			const embed = new Discord.MessageEmbed()
			.setColor(0x000000)
			.setDescription(`
	...
	async slashExecute(client, Discord, interaction, serverDoc){
		...
		let mode = value.startsWith('+') ? 'add' : value.startsWith('-') ? 'subtract' : 'set';
