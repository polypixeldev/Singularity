module.exports = {
	...
	async slashExecute(client, Discord, interaction, serverDoc){
		let userMS = await client.utils.loadMsInfo(client, serverDoc, interaction.user.id);
		let limit = -1;
		let remaining = {
			protons: userMS.protons,
			electrons: userMS.electrons
		...
		.setDescription(`
			...
			*Respond with the desired number of upgrades within 30 seconds, or respond with 0 to abort*
		`)
		.setFooter(`Upgrade info requested by ${interaction.user.tag} • ${currentDate.getUTCMonth()}/${currentDate.getUTCDate()}/${currentDate.getUTCFullYear()} @ ${currentDate.getUTCHours()}:${currentDate.getUTCMinutes()} UTC`, interaction.user.displayAvatarURL());
		interaction.editReply({embeds: [embed]});

		interaction.channel.awaitMessages(message => message.author.id === interaction.user.id && Number.isInteger(Number(message.content)), {max: 1, time: 30000, errors: ['time']})
		.then(collection => {
			let num = Number(collection.first().content)
			if(num > limit){
				const embed = new Discord.MessageEmbed()
