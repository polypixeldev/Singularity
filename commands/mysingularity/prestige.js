module.exports = {
	...
	options: [{
		name: 'info',
		description: 'Enter "info" to display information about Singularity Prestige',
		type: 'STRING',
		required: false
	}],
	args: [],
	aliases: [],
	example: 'ms prestige',
	async execute(client, Discord, msg, args, serverDoc, rareItems){
	...
	async slashExecute(client, Discord, interaction, serverDoc, rareItems){
		...
		if(interaction.options.get('info') === 'info'){
			...
			.setDescription(`
				...
				*Your Lifetime Experience will not be affected*
			`)
			.setFooter(`Singularity Prestige Info requested by ${interaction.user.tag} • ${currentDate.getUTCMonth()}/${currentDate.getUTCDate()}/${currentDate.getUTCFullYear()} @ ${currentDate.getUTCHours()}:${currentDate.getUTCMinutes()} UTC`, interaction.user.displayAvatarURL());
