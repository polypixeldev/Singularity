module.exports = {
	...
	async slashExecute(client, Discord, interaction, serverDoc, items, powerUps){
		...
		if(!userMS.items.includes(interaction.options.get('item').value) && !userMS.powerUps.includes(interaction.options.get('item').value)){
			const embed = new Discord.MessageEmbed()
			.setColor(0x000000)
			.setDescription('You do not own this item!');
		...
		if(selectedItem[0] === 'item'){
			for(let i=0; i < userMS.items.length; i++){
				if(userMS.items[i] === interaction.options.get('item').value && removed <= quantity){
					userMS.items.splice(i, 1);
					removed++;
				}
			}
			...
			for(let i=0; i < userMS.powerUps.length; i++){
				if(userMS.powerUps[i] === interaction.options.get('item').value && removed <= quantity){
					userMS.powerUps.splice(i, 1);
					removed++;
				}
			}
