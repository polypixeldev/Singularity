...
module.exports = {
	...
	async slashExecute(client, Discord, interaction, serverDoc, items, powerUps){
		if(interaction.options.getSubcommand(false) === 'buy'){
			buy.slashExecute(client, Discord, interaction, serverDoc, items, powerUps)
		} else if(interaction.options.getSubcommand(false) === 'sell'){
			sell.slashExecute(client, Discord, interaction, serverDoc, items, powerUps)
		} else {
			let itemStr = ""
			for(let item in items){
				itemStr = itemStr + `**${items[item].name}**: *${items[item].protons} Protons, ${items[item].electrons} Electrons, ${items[item].darkMatter} Dark Matter* \n`
			...
			.setDescription(`
				...
				*Use \`${serverDoc.prefix}singularity shop buy "<item_name>" <!quantity>\` to buy an item!*
			`)
			.setFooter(`Singularity Shop requested by ${interaction.user.tag} • ${currentDate.getUTCMonth()}/${currentDate.getUTCDate()}/${currentDate.getUTCFullYear()} @ ${currentDate.getUTCHours()}:${currentDate.getUTCMinutes()} UTC`, interaction.user.displayAvatarURL());
