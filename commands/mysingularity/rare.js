module.exports = {
	...
	async slashExecute(client, Discord, interaction, serverDoc, rareItems){
		...
		.setTitle(`My Singularity - Rare Items`)
		.setDescription('These items are **rare**, and cannot be bought from the Singularity shop. Instead, you have a chance to find them when doing special actions, such as prestiging.')
		.setFooter(`Rare item list requested by ${interaction.user.tag} • ${currentDate.getUTCMonth()}/${currentDate.getUTCDate()}/${currentDate.getUTCFullYear()} @ ${currentDate.getUTCHours()}:${currentDate.getUTCMinutes()} UTC`, interaction.user.displayAvatarURL())
		for(let item of rareItems){
			embed.addField(item.name, item.description);
		}
