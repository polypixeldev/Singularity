module.exports = {
    ...
    async slashExecute(client, Discord, interaction){
      ...
      if(interaction.options.get('tag').value === 'list'){
        ...
        .setDescription(banListStr)
        .setFooter(`Ban list requested by ${interaction.user.tag}`, interaction.user.displayAvatarURL());

        return interaction.editReply({embeds: [listEmbed]});
      }

      const banInfo = bans.find(ban => ban.user.tag === interaction.options.get('tag').value);
      ...
      if (banInfo) {
          ...
          .unban(user, reason ?? `${user.tag} unbanned by ${interaction.user.tag}`)
          .then(() => {
            const successEmbed = new Discord.MessageEmbed()
            .setDescription(`Successfully unbanned **${user.tag}**`)
            .setColor(0x000000);
