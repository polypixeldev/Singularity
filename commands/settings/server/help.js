...
module.exports = {
	...
	slashExecute(client, Discord, interaction, serverDoc){
		if(interaction.options.getSubcommand(false) === 'welcomemessage'){
			...
			.setDescription(`
				**Set/Toggle a Welcome Message:** \`${serverDoc.prefix}settings server welcome <channel> <message>\`
				 *- Current Setting:* \`${serverDoc.welcomeMessage}\` *in* \`${interaction.guild.channels.resolve(serverDoc.welcomeChannelID).name}\`
				**Set/Toggle a Leave Message:** \`${serverDoc.prefix}settings server leave <channel> <message>\`
				 *- Current Setting:* \`${serverDoc.leaveMessage}\` *in* \`${interaction.guild.channels.resolve(serverDoc.leaveChannelID).name}\`
			`)
			.setFooter(`Singularity Server Settings requested by ${interaction.user.tag} • ${currentDate.getUTCMonth()}/${currentDate.getUTCDate()}/${currentDate.getUTCFullYear()} @ ${currentDate.getUTCHours()}:${currentDate.getUTCMinutes()} UTC`, interaction.user.displayAvatarURL());
