		...
		if(interaction.options.getSubcommand(false) === 'prefix'){
			...
			.setDescription(`
				**Change Prefix:** \`${serverDoc.prefix}settings bot prefix <prefix>\`
				 *- Current Setting:* \`${serverDoc.prefix}\`
				**Nickname Me:**  \`${serverDoc.prefix}settings bot nickname <nickname>\`
				 *- Current Setting:* \`${interaction.guild.members.resolve(client.user).nickname ? interaction.guild.members.resolve(client.user).nickname : 'None'}\`
				**Kick Me:** \`${serverDoc.prefix}settings bot kick\`
			`)
			.setFooter(`Singularity Bot Settings requested by ${interaction.user.tag}  • ${currentDate.getUTCMonth()}/${currentDate.getUTCDate()}/${currentDate.getUTCFullYear()} @ ${currentDate.getUTCHours()}:${currentDate.getUTCMinutes()} UTC`, interaction.user.displayAvatarURL());
