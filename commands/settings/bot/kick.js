module.exports = {
	...
	slashExecute (client, Discord, interaction){
		...
		interaction.channel.awaitMessages({filter: message => message.author.id === interaction.user.id && (message.content === 'Y' || message.content === 'N'), max: 1, time: 30000, errors: ['time']})
		.then(collection => {
			let message = collection.first();
			if(message.content === 'Y'){
				let currentDate = new Date(Date.now())
				...
				.setDescription(`
					...
					*Sincerely,
					The Singularity Team*
				`)
				.setFooter(`Singularity was kicked by ${interaction.user.tag}  • ${currentDate.getUTCMonth()}/${currentDate.getUTCDate()}/${currentDate.getUTCFullYear()} @ ${currentDate.getUTCHours()}:${currentDate.getUTCMinutes()} UTC`, interaction.user.displayAvatarURL());

				interaction.editReply({embeds: [embed]}).then(() => {
					interaction.guild.leave();

					setTimeout(async () => {
						if(await client.guilds.cache.find(guild => guild.id === interaction.guild.id)){
