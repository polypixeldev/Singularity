module.exports = {
	...
	async slashExecute(client, Discord, interaction, serverDoc){
		...
		**Notes:**
		\`\`\`${command.notes ? command.notes : 'none'}\`\`\`
		`)
		.setFooter(`Arguments marked with ! are optional - Multi-word arguments should be surrounded with doublequotes - command info requested by ${interaction.user.tag} • ${currentDate.getUTCMonth()}/${currentDate.getUTCDate()}/${currentDate.getUTCFullYear()} @ ${currentDate.getUTCHours()}:${currentDate.getUTCMinutes()} UTC`, interaction.user.displayAvatarURL());
